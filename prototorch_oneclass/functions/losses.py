...
def one_class_classifier_triplet_loss(distances, target_labels,
                                      prototype_labels, theta_boundary):
    ...
    dp, dm = _get_dp_dm(diff_to_thresh,
                        target_labels,
                        prototype_labels,
                        fill_value=0)
    mu = dp - dm
...
def _test_one_class_classifier_triplet_loss(distances, target_labels,
                                            prototype_labels, theta_boundary):
    """ triplet loss for OneClassClassifier """
    zero = torch.tensor(0).type(torch.float)
    diff_to_thresh = torch.subtract(distances, theta_boundary)
...
def _backup_one_class_classifier_loss(distances, target_labels,
                                      prototype_labels, theta_boundary):
    ...
    muf = d_op_in * torch.pow(-1.,
                              torch.LongTensor(not_matcher.type(torch.long)))
    #muf += matcher * 100
...
def occ_mitRonny(
    ...
    winning_indices = torch.min(distances,
                                dim=1).indices  # Liste an Winning protos
    """
...
def occ_studentT_loss(
    ...
    # filter FP, FN
    _, _, FP, FN = error_type_determination(distances, theta_boundary,
...
def occ_loss(distances, target_labels, prototype_labels, theta_boundary):
    ...
    alpha = torch.where(alpha >= 0.5,
                        torch.tensor(0.5).type(torch.float), alpha)
    alpha = 0.2
...
def _backup_occ_loss(distances, target_labels, prototype_labels,
                     theta_boundary):
    """ OneClassClassifier loss function """
    zero = torch.tensor(0).type(torch.float)
    matcher = _get_matcher(target_labels, prototype_labels)
