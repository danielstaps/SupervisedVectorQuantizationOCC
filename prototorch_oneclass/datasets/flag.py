...
def make_flag(num_samples=300, dimensions=2, num_classes=1, blobs_per_class=1, elipses=True):
    ...
    blob_mean = np.random.uniform(
        low=0.1, high=0.9, size=(num_classes * blobs_per_class, dimensions)
    )
    # blob_mean = np.random.uniform(low=0.5, high=0.5, size=(num_classes*blobs_per_class, dimensions))
    # blob_mean[:,0] = 0.2
    blob_bord = np.random.uniform(
        low=0.05 * dimensions * test,
        high=0.15 * dimensions * test,
        size=(num_classes * blobs_per_class, dimensions),
    )
    ...
    for target in range(num_classes):
        for blob in range(blobs_per_class):
            ...
            in_border = np.where(distances < 1.0)[0]
            y[in_border] = target
...
class Flag(torch.utils.data.TensorDataset):
    ...
    def __init__(
        ...
        x, y = make_flag(num_samples, dimensions, num_classes, blobs_per_class, elipses)
